...
- **Container owns nothing but position, the size box, and show/hide.** No round look, no colour, no kind-specific CSS. Positioning uses the CSS `translate` property (from `--preview-x/y`) so `scale` + `opacity` animate on their own without a `transform` shorthand conflict — one node, position still GPU-composited for cursor/scroll follow.
- **Show/hide is a CSS transition on scale + opacity only** (never geometry). It is a trivial fade, and a CSS transition interrupts cleanly on a fast hover in/out; WAAPI via `animationManager` was tried here first but its double-rAF gate + fill-forwards commit stuttered on rapid interaction. WAAPI is reserved for the one genuinely complex, sequenced animation: the album disc.
...
- **Sequencing via promises, not timers.** `PreviewContainer.hide()` returns a promise resolved by `transitionend` (opacity), with a computed-style-derived fallback timer for a missed event — so retract → teardown → reveal is `await`-ed rather than timed with `SWAP_MS`/`HIDE_MS`. The *decision* of when to sequence this way — Hover intent, Warm state, the shape-swap defer, generation superseding — lives in a pure `PreviewChoreography` state machine (`previewChoreography.ts`); the singleton only feeds it events and executes the commands it returns.
