  ...
  protected render(): unknown {
    ...
    ? html`
      <button type="button"
        ...
        class="sw:flex sw:items-center sw:gap-2 sw:text-sm sw:min-w-[75px] sw:uppercase sw:font-sans sw:font-light sw:tracking-wider sw:text-inherit sw:hover:text-accent sw:transition-colors sw:cursor-pointer"
        @click=${this._handleUserToggle}>
        
        <span 
          class="${isTerminal === "terminal" ? "hidden" : ""} dot-container ${isDark ? 'sw:before:scale-100' : 'sw:before:scale-0'} sw:before:content-[''] sw:before:w-2 sw:before:h-2 sw:before:rounded-full sw:before:bg-accent sw:before:transition-transform sw:before:duration-300">
        </span>
    ...
    : html`
      <button type="button"
        aria-label=${isDark ? "Switch to light theme" : "Switch to dark theme"}
        aria-pressed=${String(isDark)}
        class="sw:p-0 sw:flex sw:items-center sw:gap-2 sw:text-sm sw:min-w-[75px] sw:uppercase sw:font-sans sw:font-light sw:tracking-wider sw:text-inherit sw:hover:text-accent sw:transition-colors sw:cursor-pointer"
        @click=${this._handleUserToggle}>
