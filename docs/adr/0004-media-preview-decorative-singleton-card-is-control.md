...
- Anchoring uses fixed positioning relative to the trigger rect at commit time, so scrolling would otherwise detach the ball from its card. Resolved by input type: on **touch** the singleton follows its source card on scroll (live rect, vertical-unclamped so it trails off 1:1, an `is-tracking` class dropping the transform easing) and stops committed playback once the card is fully off-screen; on **desktop** any real scroll past a small px threshold dismisses playback. The singleton fires a `sharedpreview:stopped` window event so a card whose media was stopped out from under it (scroll-out, dismiss, or another card taking over) can drop its `aria-pressed`.
