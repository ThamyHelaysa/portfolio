...
export class MenuMobile extends LitElement {
  ...
  protected render(): unknown {
    ...
    return html`
      <button
        class="btn sw:md:hidden sw:text-accent sw:font-sans sw:text-sm sw:font-bold sw:uppercase sw:tracking-widest sw:px-2 sw:py-1 sw:hover:opacity-70 sw:transition-opacity"
      ...
      <div
        id="mobile-menu"
        class="sw:inset-0 sw:z-99 sw:bg-background sw:transition-colors sw:duration-500 sw:ease-in-out sw:md:hidden sw:flex sw:flex-col "
        role="dialog"
        aria-modal="true"
        aria-label="Navigation Menu"
      >
        <div class="sw:px-6 sw:py-6 sw:flex sw:justify-between sw:items-center sw:border-b sw:border-accent/5">
          <span class="sw:text-accent sw:font-serif sw:text-xl sw:tracking-tight">Menu</span>
          <button
            @click="${this._handleOpen}"
            class="btn sw:text-accent sw:font-sans sw:text-sm sw:font-bold sw:uppercase sw:tracking-widest sw:p-2"
        ...
        <nav 
          ...
          <div class="sw:pt-12 sw:px-8 sw:gap-8 sw:border-t sw:border-accent/5 sw:transition-transform sw:transition-opacity sw:duration-500 sw:delay-600 ${open ? 'sw:opacity-100 sw:translate-x-0' : 'sw:opacity-0 sw:-translate-x-4'
      }">
            <div class="sw:flex sw:flex-col sw:gap-3">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                class="sw:py-3 sw:text-lg sw:font-serif sw:italic sw:text-accent/80 sw:hover:text-accent"
              >
                LinkedIn &rarr;
              </a>
              <slot name="linkedin"></slot>
              <div class="sw:py-3 sw:flex sw:items-center sw:gap-4 sw:text-accent">
